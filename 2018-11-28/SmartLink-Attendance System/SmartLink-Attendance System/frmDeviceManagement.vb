...
Public Class frmDeviceManagement
    ...
    Private Sub txtSearchArea_KeyDown(sender As Object, e As KeyEventArgs) Handles txtSearchArea.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then
                If txtSearchArea.Text = "" Then
                    showDGVArea()
                    conn.Close()
                Else
                    openConnection()
                    Dim adaptor As New SqlDataAdapter("SELECT areaNumber 'Area Number', areaName 'Area Name', remarks 'Remarks' FROM tArea WHERE areaNumber LIKE '%" + txtSearchArea.Text + "%" + "' or areaName like '%" + txtSearchArea.Text + "%" + "' or remarks LIKE '%" + txtSearchArea.Text + "%" + "'", conn)
                    Dim ds As New DataSet
                    adaptor.Fill(ds, "Data")
                    dgvArea.DataSource = ds.Tables("Data")
                    conn.Close()
                End If
            End If
        Catch ex As Exception
