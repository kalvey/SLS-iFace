...
Public Class frmEmpContract
    ...
    Sub showDGVArea()
        Try
            ...
            Dim adaptor As New SqlDataAdapter(
                ...
                departmentName 'Department Name', position 'Position', section 'Section', tUserExternal.subcon 'Subcon', contractStartDate 'Contract Start Date',
                contractEndDate 'Contract End Date', datediff(dd,current_timestamp,contractEndDate) 'Remaining Work Days'
                FROM tDeviceUser2 JOIN tUserExternal ON tDeviceUser2.userId = tUserExternal.userId
                WHERE datediff(dd, current_timestamp,contractEndDate) <= 3 AND [status] = 'ACTIVE'
    ...
    Sub getTotalAttendance()
        ...
        For t As Integer = 0 To dgvAbsence.Rows.Count - 1
            ...
            For k As Integer = 7 To dgvAbsence.Columns.Count - 2
            ...
            dgvAbsence.Rows(t).Cells(7 + System.DateTime.DaysInMonth(DateTime.Now.Year, DateTime.Now.Month)).Value = total

            Dim dtNow As Date = Date.Now
            Dim dtFirstOfMonth As Date = dtNow.AddDays(-dtNow.Day + 1)
            dgvLog.Rows.Add(dgvAbsence.Rows(t).Cells(0).Value, dgvAbsence.Rows(t).Cells(1).Value, dgvAbsence.Rows(t).Cells(2).Value, dgvAbsence.Rows(t).Cells(3).Value, dgvAbsence.Rows(t).Cells(4).Value, dgvAbsence.Rows(t).Cells(5).Value, dgvAbsence.Rows(t).Cells(6).Value, (DateDiff(DateInterval.Day, dtFirstOfMonth, DateTime.Now) - total))
        Next
    ...
    Private Sub frmEmpContract_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        adaptor = New SqlDataAdapter("
            ...
            SELECT  tDeviceUser2.userId, tDeviceUser2.employeeNo 'Employee No', tDeviceUser2.employeeName 'Employee Name', tDeviceUser2.departmentName 'Department Name', position 'Position', section 'section', tUserExternal.subcon 'Subcon'," & result & "
            FROM #tTemp LEFT JOIN tDeviceUser2 ON tDeviceUser2.userId = #tTemp.userId 
            LEFT JOIN tUserExternal ON tUserExternal.userId = tDeviceUser2.userId
            DROP TABLE #tTemp
